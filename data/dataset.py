...
class PDBBindBenchmark(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file):
        super().__init__()
        self.drop_num=0
        self.df = pd.read_csv(f'{data_file}/label.csv', dtype=str)
        self.apo_pocket = open_data_file(f'{data_file}/apo_pocket.pkl')
        self.apo_ligand = open_data_file(f'{data_file}/apo_ligand.pkl')
        self.holo_pocket = open_data_file(f'{data_file}/holo_pocket.pkl')
        self.holo_ligand = open_data_file(f'{data_file}/holo_ligand.pkl')
        self.data = []
        for i in range(len(self.df)):
            temp_list=[]
            pdb_id = self.df.iloc[i]['pdbid']
            for item in self.apo_pocket:
                if pdb_id in item['id']:
                    temp_list.append(item)
                    break
            for item in self.apo_ligand:
                if pdb_id in item['id']:
                    temp_list.append(item)
                    break
            for item in self.holo_pocket:
                if pdb_id in item['id']:
                    temp_list.append(item)
                    break
            for item in self.holo_ligand:
                if pdb_id in item['id']:
                    temp_list.append(item)
                    break
            if len(temp_list) == 4:
                data = {
                    'pdb_id': pdb_id,
                    'apo_pocket': temp_list[0]['data'],
                    'apo_ligand': temp_list[1]['data'],
                    'holo_pocket': temp_list[2]['data'],
                    'holo_ligand': temp_list[3]['data'],
                    'label': self.df.iloc[i]['-logKd/Ki']
                }
                if len(data['apo_pocket']['X'])+len(data['apo_ligand']['X']) == len(data['holo_pocket']['X'])+len(data['holo_ligand']['X']) and len(data['apo_pocket']['B'])+len(data['apo_ligand']['B']) == len(data['holo_pocket']['B'])+len(data['holo_ligand']['B']):
    ...
    def collate_fn(cls, batch):
        ...
        # 只保留 apo 分支，holo 分支改为一个合并后的 'holo'
        input_types = ['apo_pocket', 'apo_ligand', 'holo']

        # 遍历三种输入类型
        for itype in input_types:
            # 对于 holo，我们要把 pocket 和 ligand 先在单个样本里合并
            if itype == 'holo':
                # 对每个样本，先拼接 pocket + ligand
                temp_batch = []
                for item in batch:
                    pocket = item['holo_pocket']
                    ligand = item['holo_ligand']

                    # 合并特征 X, B, A
                    combined = {
                        'X':    np.concatenate([pocket['X'],    ligand['X']],    axis=0),
                        'B':    np.concatenate([pocket['B'],    ligand['B']],    axis=0),
                        'A':    np.concatenate([pocket['A'],    ligand['A']],    axis=0),
                    }
                    # block_lengths 本身是 list，直接拼 list
                    combined['block_lengths'] = pocket['block_lengths'] + ligand['block_lengths']

                    # 新的 segment_ids：前 len(pocket_blocks) 个标 0，后 len(ligand_blocks) 个标 1
                    num_p = len(pocket['block_lengths'])
                    num_l = len(ligand['block_lengths'])
                    combined['segment_ids'] = [0] * num_p + [1] * num_l

                    temp_batch.append(combined)
            else:
                # apo 分支按原来方式
                temp_batch = [item[itype] for item in batch]

            # 以下统一把 temp_batch 中的每个字段 concat 到一起
            keys  = ['X', 'B', 'A', 'block_lengths', 'segment_ids']
            types = [torch.float, torch.long, torch.long, torch.long, torch.long]

            res = {}
            for key, dtype in zip(keys, types):
                # 准备把每个样本对应的 key 转成 tensor 后在 batch 维度 cat
                tensors = []
                for entry in temp_batch:
                    tensors.append(torch.tensor(entry[key], dtype=dtype))
                res[key] = torch.cat(tensors, dim=0)

            # 记录每个样本原始的“B”长度，用于后续拆分
            lengths = [len(entry['B']) for entry in temp_batch]
            res['lengths'] = torch.tensor(lengths, dtype=torch.long)

            return_dict[itype] = res

        # 最后的 label 部分不变
        return_dict['label'] = torch.tensor(
            [float(item['label']) for item in batch],
            dtype=torch.float
        )
        return return_dict
